  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-8 px-6 py-12 lg:px-10">
      <section className="rounded-[28px] border border-slate-200 bg-white/90 p-8 shadow-xl shadow-slate-200/80">
        <p className="text-sm uppercase tracking-[0.35em] text-teal-700">Cache patterns · ISR / SSR / static strategies</p>
        <h1 className="mt-3 text-4xl font-semibold tracking-tight text-slate-900 md:text-5xl">Cache strategy examples</h1>
        <p className="mt-3 text-slate-600">This page demonstrates three practical cache patterns: force-cache for static content, no-store for always-fresh data, and ISR for periodic background refreshes. It is designed to make cache decisions obvious and explainable.</p>
        <p className="mt-3 rounded-2xl bg-slate-50 p-3 text-sm text-slate-700">Architecture use: fetch caching, revalidation, server-side data freshness, and explicit strategy choices for real-world rendering tradeoffs.</p>
      </section>
